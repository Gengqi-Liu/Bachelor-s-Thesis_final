...
$\mathbf{x}_{\mathrm{tx}} \in \mathbb{R}^{N_{\mathrm{samples}} \times N_{\mathrm{T}}}$
...
$\mathbf{y}_{\mathrm{rx}} \in \mathbb{R}^{L \times N_{\mathrm{R}}}$,
...
Field & Description \\
\hline
\texttt{rxSignal} & Recorded signal matrix $[N_\text{samp} \times N_{\mathrm{R}}]$ \\
\texttt{fs} & Sampling rate ($f_{\mathrm{s}}$) \\
...
\Ensure \texttt{mFrameRxNoCP} $[N_{\mathrm{FFT}} \times N_{\mathrm{blocks}} \times N_{r}]$ and \texttt{MetricData} per channel
...
\State Cut \texttt{iNewNoBlocks} blocks of length \texttt{iNb}, reshape to \texttt{mFrameRx} $[iNb \times iNewNoBlocks \times N_{r}]$
