...
For the considered configuration with $N_{\mathrm{T}} = 4$ transmit channels and $N_{\mathrm{R}} = 8$ receive channels, the rank is equal to four for all subcarriers. This is further confirmed by the rank histogram shown in the top-right panel, where all observations fall into a single bin centered at rank four. No rank deficiency is observed across the evaluated bandwidth.
...
\[
\mathrm{BER}=\frac{N_{\mathrm{err}}}{N_{\mathrm{comp}}}, \qquad N_{\mathrm{comp}}=\min\{N_{\mathrm{T}},N_{\mathrm{R}}\}.
\]
